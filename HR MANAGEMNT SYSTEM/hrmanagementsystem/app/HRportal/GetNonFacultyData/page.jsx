...
function page() {
    ...
    useEffect(() => {
      ...
      if (!token) {
          ...
          fetch('/api/verifyOtp', {
              ...
              .then((res) => {
                  if (res.status !== 200) {
                      router.push('/HRportal/GetNonFacultyData');
                  }
              })
              .catch(() => router.push('/'));
      }
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await fetch('/api/GetNonFacultyData');
                if (!response.ok) {
    ...
    return (
        <div className="p-6">
            <h1 className="text-3xl text-center font-bold mb-4">Non Faculty Employee Details</h1>
